...
// const rasa_server_url = "https://trialbot-guidona.herokuapp.com:5005/webhooks/rest/webhook";
const rasa_server_url = "http://localhost:5005/webhooks/rest/webhook";
// const rasa_server_url = "http://13.58.89.188:5005/webhooks/rest/webhook";
...
function showBotTyping() {
  const botTyping =
    '<img class="botAvatar" id="botAvatar" src="img/chatboticon.jpeg"/><div class="botTyping"><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div>';
...
function setBotResponse(response) {
  ...
  setTimeout(() => {
    ...
    if (response.length < 1) {
      ...
      const BotResponse = `<img class="botAvatar" src="img/chatboticon.jpeg"/><p class="botMsg">${fallbackMsg}</p><div class="clearfix"></div>`;
      ...
      for (let i = 0; i < response.length; i += 1) {
        ...
        if (Object.hasOwnProperty.call(response[i], "text")) {
          if (response[i].text != null) {
            const BotResponse = `<img class="botAvatar" src="img/chatboticon.jpeg"/><p class="botMsg">${response[i].text.replace(/(?:\r\n|\r|\n)/g, '<br>')}</p><div class="clearfix"></div>`;
            $(BotResponse).appendTo(".chats").hide().fadeIn(1000);
          }
        }
        ...
        if (Object.hasOwnProperty.call(response[i], "custom")) {
          ...
          if (payload === "cardsCarousel") {
            const rData = response[i].custom.data;
            showCardsCarousel(rData);
            return;
          }
...
function send(message) {
  ...
  let from_get = [
    ...
    from_get[0].text=message;

  $.ajax({
    url: rasa_server_url,
    type: "POST",
    contentType: "application/json",
    data: JSON.stringify({ message, sender: sender_id, csrfmiddlewaretoken: $('input[name=csrfmiddlewaretoken]').val()}),
    success(botResponse, status) {
      console.log("Response from Rasa: ", botResponse, "\nStatus: ", status);
